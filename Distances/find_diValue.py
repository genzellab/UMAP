...
ratsArray=[value_cmp("subject", 1),value_cmp("subject", 2),value_cmp("subject", 3),
           value_cmp("subject", 4),value_cmp("subject", 6),value_cmp("subject", 7),
           value_cmp("subject", 8),value_cmp("subject", 9)]
...
def find_logic(trial, rat, day):
    K=[]
    
    for i in range(len(trial)):
        for j in range(len(rat)):
            for k in range(len(day)):
                logicresult=trial[i]*rat[j]*day[k]
...
def outputDI():
    res=[]
    logicresult=find_logic(trialsArray, ratsArray, daysArray)
    for i in range(144):
        for j in range(144):
