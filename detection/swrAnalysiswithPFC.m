...
% start
% 260 Rat 1, CON
% Time to seconds + offset 2 elimination
%rat 4 d 4  with different STDs 10-5-6
%1x1 cell	1x1 cell	211x6 cell	188x6 cell	32x6 cell	307x6 cell	277x6 cell	253x6 cell	133x6 cell
%1x1 cell	1x1 cell	259x6 cell	249x6 cell	38x6 cell	419x6 cell	359x6 cell	336x6 cell	178x6 cell
%1x1 cell	1x1 cell	247x6 cell	235x6 cell	38x6 cell	401x6 cell	342x6 cell	322x6 cell	166x6 cell
% {'1'}    {'11'}    {'13'}    {'3'}    {'4'}    {'6'}    {'9'}
tic
clear variables
clc
...
% singletrialdetection=0;
% rat 3 sd 2 new counts for ripples with std7 0	0	0	50	46	89	158	180	176
...
k=7; % Rat number (This is not the actual number of the animal). This is with respect to "rat_folder".
% while k<6
% for k=1:length(rat_folder)
    %rat index 
    cd(rat_folder{k})    
...
Cortex=Cortex.*(0.195);
        ...
        if sum(states==3)< 180 % If there's less than 3 min of NREM
...
corrected_means = []; 
 ...
 TT.corrected_means = num2cell(corrected_means);
       ...
       cd(g{j})
% if j==3 & k==3 & singletrialdetection==1
%     TT=load("TT.mat")
%     TT=TT.TT;
% end
...
[D1, swr_hpc,swr_pfc,s_hpc,s_pfc,V_hpc,V_pfc,signal2_hpc,signal2_pfc,sd_swr, M_multiplets, Mx_multiplets, multiplets, ripples, Mono_hpc, Mono_pfc,total_swrs,total_NREM_min ]=ripple_detectionPFC(HPC,Cortex,states,ss,offset1,offset2,TT,fn);
...
%        GC_window_ripples_broadband{c,1} = concatenated_NREM_hpc_broadband(int32(ripples(c,6)*fn+1-(3*fn)) : int32(ripples(c,6)*fn+1+(3*fn)));
       ...
       GC_window_ripples_broadband{c,7} = D1;
...
%                        clear HPC_1 HPC_2 HPC_3 HPC_4                                                     
                        ...
                        if length(Cortex)<45*60*fn*4
                          ...
                          [D1,swr_hpc, swr_pfc, s_hpc, s_pfc, V_hpc, V_pfc, signal2_hpc, signal2_pfc, sd_swr, M_multiplets, Mx_multiplets, multiplets, ripples, Mono_hpc, Mono_pfc,total_swrs,total_NREM_min]=ripple_detectionPFC(hpc,pfc,states_chunk,ss,offset1,offset2,TT,fn); 
