...
% folderpath_raw_data='/media/genzel/My Passport1/ephys';
% folderpath_raw_data='/media/genzel/My Passport/ephys/'; % Rats 9 and 11.
% folderpath_raw_data='/mnt/genzel/Rat/OS/OS_rat_ephys' % Rat 13
folderpath_downsampled_data='F:\OSF\UMAP';
folderpath_raw_data='\\genzel-srv.science.ru.nl\genzel\Rat\OS\Rat_OS_Ephys_CBD';

cd(folderpath_downsampled_data)
load('OS_CBD_CHRONIC.mat');

labelconditions=labelconditions([1 2 3]);
labelconditions2=labelconditions2([1 2 3]);


% rats=[1 3 4 6 9 11 13];
fs_rats=[ones(1,4)*30000 ];
...
dname2=uigetdir(folderpath_downsampled_data,strcat('Select folder where downsampled data should be saved'));
%%
%SELECT CONDITION.
...
% if size(label1,1)~=3 %Not Plusmaze
    ...
    if contains(BB,'FOR') %Ignore the FORaging condition
        iii=iii+1;
        continue
    end
    cd(BB)
...
%%
%xo
%\\genzel-srv.science.ru.nl\genzel\Rat\OS\Rat_OS_Ephys_CBD\Rat_OS_Ephys_CBD_rat3_407698\Rat_OS_Ephys_cbd_chronic_Rat3_407698_SD10_HC_20210621\Recordings\2021-06-21_11-58-13_sleep
...
try
    ...
    [PAR, ~, ~] = load_open_ephys_data(cf3{1});
